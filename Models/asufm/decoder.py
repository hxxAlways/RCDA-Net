...
from Models.asufm.swin_block import SwinTransformerBlock, SwinTransformerBlockFM
from Models.asufm.skipconnection.crossvit import CrossTransformer
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(
        ...
        # build blocks
        if focal is True:
            # We don't use focal modulation for decoder
            self.blocks = nn.ModuleList(
                [
                    SwinTransformerBlockFM(
                        dim=dim,
    ...
    def forward(self, x, y):
        def checkpoint_fwd(blk, x, y):
            x, _ = blk(x, ["decoder", y])
            return x

        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(checkpoint_fwd, blk, x, y)
            else:
                x, _ = blk(x, ["decoder", y])
...
class Decoder(nn.Module):
    def __init__(
        ...
        if self.mode in ["cross_contextual_attention"]:
            self.Crossvit_layer = nn.ModuleList()
            for i in range(self.num_layers - 1):
                self.Crossvit_layer.append(
                    CrossTransformer(
                        sm_dim=embed_dim * 2**i,
                        lg_dim=embed_dim * 2**i,
                        depth=2,
                        heads=8,
                        dim_head=64,
                        dropout=drop_rate,
