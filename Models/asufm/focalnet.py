# --------------------------------------------------------
# Based on the work of https://github.com/givkashi/Focal-UNet
# and
# FocalNets -- Focal Modulation Networks
# Copyright (c) 2022 Microsoft
# Licensed under The MIT License
# Modified by Bronte Sihan Li
# --------------------------------------------------------
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            print(
                ...
                sum(
                    p.numel()
                    for p in self.layers[i_layer].parameters()
                    if p.requires_grad
                )
                // 10**6,
            )
        ...
        for i_layer in range(self.num_layers - 1, -1, -1):
            concat_linear = (
                nn.Linear(
                    2 * int(self.embed_dim[0] * 2**i_layer),
                    int(self.embed_dim[0] * 2**i_layer),
                )
                if i_layer < 3
                else nn.Identity()
            )
            if i_layer == self.num_layers - 1:
                layer = PatchExpand(
                    ...
                    patch_size=2,
                    in_chans=embed_dim[i_layer],
                    embed_dim=embed_dim[i_layer - 1],
                    norm_layer=norm_layer,
                )
            else:
                layer = BasicLayer(
...
def build_transforms(img_size, center_crop=False):
    ...
    if center_crop:
        ...
        t.append(transforms.Resize(size, interpolation=str_to_pil_interp('bicubic')))
        t.append(transforms.CenterCrop(img_size))
    else:
        t.append(transforms.Resize(img_size, interpolation=str_to_pil_interp('bicubic')))
...
def focalnet_tiny_srf(pretrained=False, **kwargs):
    model = FireFocalNet(depths=[3, 3, 3, 3], embed_dim=96, **kwargs)
    if pretrained:
...
def focalnet_small_srf(pretrained=False, **kwargs):
    model = FireFocalNet(depths=[2, 2, 18, 2], embed_dim=96, **kwargs)
    if pretrained:
...
def focalnet_base_srf(pretrained=False, **kwargs):
    model = FireFocalNet(depths=[2, 2, 18, 2], embed_dim=128, **kwargs)
    if pretrained:
...
def focalnet_tiny_lrf(pretrained=False, **kwargs):
    model = FireFocalNet(depths=[4, 4, 4, 4], embed_dim=96, **kwargs)
    if pretrained:
        url = model_urls['focalnet_tiny_lrf']
        checkpoint = torch.load(
            r"./pretrained_focal/focalnet_tiny_lrf.zip", map_location="cpu"
        )
        model.load_state_dict(checkpoint["model"], strict=False)
    return model
