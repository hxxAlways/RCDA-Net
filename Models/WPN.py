...
# Article from: https://doi.org/10.1145/3577190.3614116
...
class WPN(nn.Module):
    ...
    def forward(self, x):
        # encoding path
        x1 = self.Conv1(x) # batch * 64 * 32 * 32

        x2 = self.Maxpool(x1) # batch * 64 * 16 * 16
        x2 = self.Conv2(x2) # batch * 128 * 16 * 16

        x3 = self.Maxpool(x2) # batch * 128 * 8 * 8
        x3 = self.Conv3(x3) # batch * 256 * 8 * 8

        x4 = self.Maxpool(x3) # batch * 256 * 4 * 4
        x4 = self.Conv4(x4) # batch * 512 * 4 * 4

        d4 = self.Up4(x4) # batch * 256 * 8 * 8
        x3 = self.Att4(g=d4, x=x3) # batch * 256 * 8 * 8
        d4 = torch.cat((x3, d4), dim=1) # batch * 512 * 8 * 8
        d4 = self.Up_conv4(d4) # batch * 256 * 8 * 8

        d3 = self.Up3(d4) # batch * 128 * 16 * 16
        x2 = self.Att3(g=d3, x=x2) # batch * 128 * 16 * 16
        d3 = torch.cat((x2, d3), dim=1) # batch * 256 * 16 * 16
        d3 = self.Up_conv3(d3) # batch * 128 * 16 * 16

        d2 = self.Up2(d3) # batch * 64 * 32 * 32
        x1 = self.Att2(g=d2, x=x1) # batch * 64 * 32 * 32
        d2 = torch.cat((x1, d2), dim=1) # batch * 128 * 32 * 32
        d2 = self.Up_conv2(d2) # batch * 64 * 32 * 32
